    ...
    <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out">
      ...
      <div className="bg-white/40 dark:bg-black/20 backdrop-blur-md rounded-2xl p-6 border border-white/20 dark:border-white/10 shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <ResultCard label="일본식 죠 (畳)" value={results.jou} active={inputType === "jou"} icon="🇯🇵" desc="1畳 ≈ 1.62m²" />
          <ResultCard label="평방미터 (m²)" value={results.m2} active={inputType === "m2"} icon="📏" desc="국제 표준 규격" />
          <ResultCard label="한국식 평 (坪)" value={results.pyeong} active={inputType === "pyeong"} icon="🇰🇷" desc="1평 ≈ 3.305m²" />
        </div>
      </div>
    </div>
  );
...
function ResultCard({ label, value, active, icon, desc }: { label: string, value: string, active: boolean, icon: string, desc: string }) {
  return (
    <div className={`p-5 rounded-2xl border transition-all duration-500 flex flex-col justify-between h-full bg-white dark:bg-gray-900 group hover:-translate-y-1 hover:shadow-lg
      ${active 
        ? "border-emerald-500 shadow-[0_0_15px_-3px_rgba(16,185,129,0.3)] ring-1 ring-emerald-500 dark:border-emerald-500" 
    ...
    >
      <div className="flex justify-between items-start mb-4">
        ...
        {active && <span className="px-2 py-1 bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-400 text-xs font-bold rounded-full">입력 기준</span>}
      </div>
      <div>
        <p className="text-3xl font-extrabold text-gray-900 dark:text-white truncate" title={value}>{value}</p>
